...
export const fetchCategory = (id) => (dispatch) => {
  return Axios.get(
    `http://localhost:3000/api/categories/find-products/${id}`
  ).then((res) => {
...
export const deleteCategories = (id) => (dispatch) => {
  return Axios.delete(`http://localhost:3000/api/categories/delete/${id}`).then(
    (res) => {
      dispatch(setCategories(res.data));
    }
  );
...
export const addCategories = (body) => (dispatch) => {
  return Axios.post(`http://localhost:3000/api/categories/add`, { body }).then(
    (res) => {
      dispatch(setCategories(res.data));
    }
  );
...
export const updateCategory = (id, body) => (dispatch) => {
  return Axios.put(`http://localhost:3000/api/categories/update/${id}`, {
    body,
  }).then((res) => {
    dispatch(setCategories(res.data));
  });
...
export const fetchCategoryPosta = (id) => (dispatch) => {
  return Axios.get(`http://localhost:3000/api/categories/${id}`).then((res) => {
