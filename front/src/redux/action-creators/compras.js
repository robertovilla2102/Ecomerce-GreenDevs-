...
export const createCompra = (carritoId, body) => dispatch => {
  return Axios.post(`http://localhost:3001/api/compra/add/${carritoId}/`, {
    body
  }).then(res => {
...
export const fetchCompras = () => dispatch => {
  Axios.get(`http://localhost:3001/api/compra`).then(res =>
    dispatch(receiveCompras(res.data))
  );
};

export const fetchCompra = id => dispatch => {
  return Axios.get(`http://localhost:3001/api/compra/find/${id}`)
    .then(res => {
